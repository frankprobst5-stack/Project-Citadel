(function () {
  ...
  const weatherRadarImg = document.getElementById("weather-radar-img");
  const weatherRadarRefresh = document.getElementById("weather-radar-refresh");
  const weatherForecastList = document.getElementById("weather-forecast-list");
  const weatherRadioBtn = document.getElementById("weather-radio-btn");
  const cloudChartGrid = document.getElementById("cloud-chart-grid");

  let weatherConfigCache = null;
  let cloudTypesCache = null;

  function renderCloudChart(clouds) {
    cloudChartGrid.innerHTML = "";
    clouds.forEach(function (cloud) {
      const wrap = document.createElement("div");

      const item = document.createElement("div");
      item.className = "cloud-chart-item";

      const thumb = document.createElement("img");
      thumb.className = "cloud-chart-thumb";
      thumb.src = "/static/img/clouds/" + cloud.image;
      thumb.alt = cloud.name;
      item.appendChild(thumb);

      const info = document.createElement("div");
      info.className = "cloud-chart-info";
      const nameRow = document.createElement("div");
      nameRow.className = "cloud-chart-name-row";
      const name = document.createElement("span");
      name.className = "cloud-chart-name";
      name.textContent = cloud.name;
      nameRow.appendChild(name);
      const altitude = document.createElement("span");
      altitude.className = "cloud-chart-altitude";
      altitude.textContent = cloud.altitude;
      nameRow.appendChild(altitude);
      info.appendChild(nameRow);
      const looks = document.createElement("div");
      looks.className = "cloud-chart-looks";
      looks.textContent = "Looks like: " + cloud.looksLike;
      info.appendChild(looks);
      item.appendChild(info);

      const desc = document.createElement("div");
      desc.className = "cloud-chart-desc";
      desc.textContent = cloud.description;
      desc.hidden = true;
      ...
      cloudChartGrid.appendChild(wrap);
    });
  }

  function refreshRadarImage() {
    if (!weatherConfigCache || !weatherConfigCache.radar_station) return;
    const url = "https://radar.weather.gov/ridge/standard/" + weatherConfigCache.radar_station + "_loop.gif";
    weatherRadarImg.src = url + "?t=" + Date.now();
  }

  async function openWeather() {
    weatherOverlay.hidden = false;

    if (!weatherConfigCache) {
      const configRes = await fetch("/api/settings");
      weatherConfigCache = await configRes.json();
      weatherLocation.textContent = weatherConfigCache.location_label
        ? "- " + weatherConfigCache.location_label
        : "- no location set (see Settings)";
      if (weatherConfigCache.weather_radio_url) {
        weatherRadioBtn.hidden = false;
        weatherRadioBtn.textContent = "🎙️ " + (weatherConfigCache.weather_radio_label || "Listen Live");
        weatherRadioBtn.addEventListener("click", function () {
          window.open(weatherConfigCache.weather_radio_url, "_blank", "noopener");
        });
      } else {
        weatherRadioBtn.hidden = true;
      }
    }
    if (!cloudTypesCache) {
      try {
        const cloudsRes = await fetch("/api/weather/clouds");
        cloudTypesCache = await cloudsRes.json();
        renderCloudChart(cloudTypesCache);
      } catch (err) {
        cloudChartGrid.innerHTML = "<div class=\"planner-notes-empty\">Couldn't load the cloud chart.</div>";
      }
    }
    refreshRadarImage();

    weatherForecastList.innerHTML = "<div class=\"planner-notes-empty\">Loading forecast...</div>";
    try {
      ...
      if (!res.ok) {
        ...
        err.className = "planner-notes-empty";
        err.textContent = data.error || "Couldn't load the forecast.";
        weatherForecastList.appendChild(err);
        return;
      }
      weatherForecastList.innerHTML = "";
      data.forEach(function (period) {
        const item = document.createElement("div");
        item.className = "weather-period";

        const left = document.createElement("div");
        const name = document.createElement("div");
        name.className = "weather-period-name";
        name.textContent = period.name;
        left.appendChild(name);
        const desc = document.createElement("div");
        desc.className = "weather-period-desc";
        desc.textContent = period.shortForecast;
        left.appendChild(desc);
        item.appendChild(left);

        const temp = document.createElement("div");
        temp.className = "weather-period-temp";
        temp.textContent = period.temperature + "°" + period.temperatureUnit;
        item.appendChild(temp);

        weatherForecastList.appendChild(item);
      });
    } catch (err) {
      weatherForecastList.innerHTML = "";
      const errEl = document.createElement("div");
      errEl.className = "planner-notes-empty";
      errEl.textContent = "Couldn't reach the weather service.";
      weatherForecastList.appendChild(errEl);
    }
  }
  ...
  weatherOverlay.addEventListener("click", function (e) {
    if (e.target === weatherOverlay) closeWeather();
  });
  weatherRadarRefresh.addEventListener("click", refreshRadarImage);
